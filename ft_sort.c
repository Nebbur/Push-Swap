...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/10/07 20:45:18 by rboia-pe          #+#    #+#             */
/*   Updated: 2023/10/07 20:45:19 by rboia-pe         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_until_pivot(t_list **stack_a, t_list **stack_b, t_data *st)
{
	while (ft_smallest_number(*stack_a)->position <= st->pivot)
	{
		if ((*stack_a)->position <= st->pivot)
			pb(stack_b, stack_a);
		else if ((*stack_a)->position > st->pivot && 
			ft_find_position_through_content(*stack_a, 
				ft_closest_to_the_first(*stack_a, st)) 
...
void	ft_sort_aa(t_list **stack_a, t_list **stack_b, t_data *st)
{
	while (ft_lstsorted(*stack_a) != 0)
	{
		if (ft_smallest_number(*stack_a)->position == (*stack_a)->position)
			pb(stack_b, stack_a);
		else if (ft_lstdistance(stack_a, 
				ft_smallest_number(*stack_a)->position, true) <= 
			(ft_lstsize(*stack_a) / 2))
			ra(stack_a, 1);
		else
			rra(stack_a, 1);
	...
	while (ft_lstsize(*stack_a) <= (st->initial_size_a / st->split))
	{
		if (ft_bigger_number(*stack_b)->position == (*stack_b)->position)
			pa(stack_a, stack_b);
		else if (ft_lstdistance(stack_b, 
				ft_bigger_number(*stack_b)->position, true) < 
...
{
	if (ft_lstsorted(*stack_a) == 0 && (ft_lstsize(*stack_a) 
			+ ft_lstsize(*stack_b)) > 5)
	{
		while ((*stack_b)->position != 
			ft_bigger_number(*stack_b)->position || 
			(*stack_b)->position != ft_2_bigger_number(*stack_b)->position)
		{
			if (ft_lstdistance(stack_b, 
					(ft_bigger_number(*stack_b)->position), true) 
				<= ft_lstsize(*stack_b) / 2)
				rb(stack_b, 1);
			else
